...
pub fun fetch(address: Address, id: UInt64): Item? {
  if let col = getAccount(address).getCapability<&MirrorItems.Collection{NonFungibleToken.CollectionPublic, MirrorItems.MirrorItemsCollectionPublic}>(MirrorItems.CollectionPublicPath).borrow() {
    if let item = col.borrowMirrorItem(id: id) {
