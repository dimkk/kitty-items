import Aka from "./Aka.cdc"
import MirrorItems from "./MirrorItems.cdc"
import FungibleToken from "./FungibleToken.cdc"
import NonFungibleToken from "./NonFungibleToken.cdc"

/*
    This is a simple MirrorItems initial sale contract for the DApp to use
    in order to list and sell MirrorItems.
...
pub contract MirrorItemsMarket {
    // SaleOffer events.
    //
    ...
    // SaleOffer
    // A MirrorItems NFT being offered to sale for a set fee paid in Aka.
    //
    pub resource SaleOffer: SaleOfferPublicView {
        // Whether the sale has completed with someone purchasing the item.
        pub var saleCompleted: Bool

        // The MirrorItems NFT ID for sale.
        pub let saleItemID: UInt64
        // The collection containing that ID.
        access(self) let sellerItemProvider: Capability<&MirrorItems.Collection{NonFungibleToken.Provider}>

        // The sale payment price.
        pub let salePrice: UFix64
        // The Aka vault that will receive that payment if teh sale completes successfully.
        access(self) let sellerPaymentReceiver: Capability<&Aka.Vault{FungibleToken.Receiver}>

        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment in Aka, and if the item is still available,
        // the MirrorItems NFT will be placed in their MirrorItems.Collection .
        //
        pub fun accept(
            buyerCollection: &MirrorItems.Collection{NonFungibleToken.Receiver},
        ...
        // to transfer the MirrorItems NFT and the capability to receive Aka in payment.
        //
        init(
            sellerItemProvider: Capability<&MirrorItems.Collection{NonFungibleToken.Provider}>,
            saleItemID: UInt64,
            sellerPaymentReceiver: Capability<&Aka.Vault{FungibleToken.Receiver}>,
            salePrice: UFix64
        ) {
            pre {
    ...
    pub resource interface CollectionManager {
        pub fun insert(offer: @MirrorItemsMarket.SaleOffer)
    ...
    pub resource Collection : CollectionManager, CollectionPurchaser, CollectionPublic {
        ...
        // purchase
        // If the caller passes a valid saleItemID and the item is still for sale, and passes a Aka vault
        // typed as a FungibleToken.Vault (Aka.deposit() handles the type safety of this)
        // containing the correct payment amount, this will transfer the MirrorItem to the caller's
        // MirrorItems collection.
        ...
        //   2. MirrorItems.Withdraw
        //   3. MirrorItems.Deposit
    ...
    init () {
        ...
        self.CollectionStoragePath = /storage/mirrorItemsMarketCollection001
        self.CollectionPublicPath = /public/mirrorItemsMarketCollection001
    }
}
